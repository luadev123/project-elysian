	...
	HelperTryAndCatch(
		function()
				...
				sharedRequires['1703a89252a94a3cb5cd02ad3d6ea64fffsd4744ee588da3340de8ca770740cc981NAzcgretsa'] = (function()
					...
					if (not isfile('EchoHub')) then
						makefolder('EchoHub');
					end;
				
					if (not isfile('EchoHub/configs')) then
						makefolder('EchoHub/configs');
					end;
					
					if (not isfile('EchoHub/configs/globalConf.bin')) then
						-- By default global config is turned on
						writefile('EchoHub/configs/globalConf.bin', 'true');
					end;
					
					local globalConfFilePath = 'EchoHub/configs/globalConf.bin';
					...
					local librarySetting = {
						...
						draggable = true,
						flags = {},
						title = string.format('EchoHub | v%s', 'DEBUG'),
						open = false,
						popup = nil,
						instances = {},
						connections = {},
						...
						tabSize = 0,
						theme = {},
						foldername =  isGlobalConfigOn and 'EchoHub/configs/global' or string.format('EchoHub/configs/%s', tostring(LocalPlayer.UserId)),
						fileext = ".json",
					...
					local Window = Library:CreateWindow("scripyty".." - "..GameName.." ".."-".." Debug" ,true)
					...
					local function ChangeRunningSpeed(toggleVal)
                        local func;
                        for i,v in next, getconnections(Events.UpdateStats.OnClientEvent) do
					...
					local function InfStam(toggleVal)
					...
					local function InfDashStam(toggleVal)
					...
					local function InfDashes(toggleVal)
						if not toggleVal then return; end
						if not loaded() then return; end
						local env = getsenv(LocalPlayer.Backpack.LocalS);
						repeat
